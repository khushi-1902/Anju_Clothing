/**
 * Seeds the products that power the "Bestsellers" section and page.
 *
 * - Reads the Shopify export (data/products_export_1.csv)
 * - Selects top curated products to mark as bestsellers
 * - Upserts them by handle
 * - Marks them isBestseller = true
 * - Ensures index on (isBestseller, createdAt DESC) for query optimization
 *
 * Usage:
...
const LIMIT = Number(process.env.SEED_LIMIT ?? 48)
...
async function main() {
  ...
  console.log(`Parsed ${products.length} bestseller products from ${path.basename(CSV_PATH)}`)
  ...
  try {
    ...
    // Ensure composite index exists for optimal performance
    await client.query(
      'CREATE INDEX IF NOT EXISTS idx_products_bestseller_created ON products ("isBestseller", "createdAt" DESC)'
    )

    // Reset previous bestsellers
    await client.query('UPDATE products SET "isBestseller" = false')
    ...
    for (const [i, p] of products.entries()) {
      ...
      const { rows } = await client.query(
        `INSERT INTO products
           (handle, name, "descriptionHtml", fabric, work, category, price, "comparePrice", "isBestseller", "createdAt")
    ...
    console.log(`\nSuccessfully seeded ${products.length} products and flagged them as bestsellers.`)
  } catch (err) {
